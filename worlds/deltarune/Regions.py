...
def DeltaruneRegion(
    world: "DeltaruneWorld",
    region_name: str,
    exits: list[str],
    locations: dict[str, LocationData | ConditionalLocationData],
) -> Region:
...
def generic_create_regions(
    world: "DeltaruneWorld",
    regions: list,
    locations: dict[str, LocationData],
    conditional_locations: dict[str, ConditionalLocationData],
):
    for region_name, exits in regions:
        locations_in_region: dict[str, LocationData | ConditionalLocationData] = {}

        locations_in_region.update([location for location in locations.items() if location[1].region == region_name])

        locations_in_region.update(
            [
                location
                for location in conditional_locations.items()
